...
export const LoginForm = ({ onForgot, onRegister, onSuccess }) => {
  ...
  const onSubmit = async (data) => {
    console.debug('[LoginForm] onSubmit called', data);
    ...
    if (res.success) {
      ...
      if (onSuccess) {
        ...
        navigate(roleRedirect[res.role] || '/dashboard');
      }
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-5'>
      ...
      <button
        type='submit'
        disabled={loading}
        onClick={() => console.debug('[LoginForm] submit button clicked')}
      ...
      <p className='text-center text-sm mt-4'>
        ...
        <button
          type='button'
          onClick={() => { console.debug('[LoginForm] switch to register clicked'); onRegister && onRegister(); }}
