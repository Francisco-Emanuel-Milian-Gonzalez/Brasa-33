...
const VALID_MODES = ['login', 'register', 'forgot'];

export const AuthPage = ({ initialMode = 'login' }) => {
  const startMode = VALID_MODES.includes(initialMode) ? initialMode : 'login';
  const [authMode, setAuthMode] = useState(startMode);
  const [floatingIcons, setFloatingIcons] = useState([]);

  useEffect(() => {
    setAuthMode(VALID_MODES.includes(initialMode) ? initialMode : 'login');
  }, [initialMode]);
  ...
  return (
    <div className='min-h-screen flex items-center justify-center p-4 relative overflow-hidden' style={{ backgroundColor: '#0D0D0D' }}>
      <Link
        to='/'
        className='absolute top-4 left-4 z-20 flex items-center gap-2 text-sm font-medium px-3 py-2 rounded-lg transition hover:bg-white/5'
        style={{ color: '#A6A6A6' }}
      >
      ...
      <div className='w-full max-w-6xl grid gap-10 lg:grid-cols-[420px_1fr] items-center z-10 relative'>
        ...
        <div className='rounded-xl shadow-lg border p-6 md:p-10' style={{ backgroundColor: '#1A1A1A', borderColor: '#333333' }}>
          <div className='flex items-center justify-center mb-2'>
            <img src={imgLogo} alt='Logo La 33' className='h-18 w-20 object-contain' />
          </div>
